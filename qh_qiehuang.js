 ...
 * 多账号：使用 & 或换行分隔
 *
 * 加密：RSA-OAEP-SHA256 + AES-256-GCM
 * 依赖：node-forge 1.3.1（运行时从固定版本 CDN 加载）
 */

const SCRIPT_NAME = "统一快乐星球茄皇（五期）";
...
function readConfig() {
  ...
  const accountArgument = cleanArgument(args[0]);
  const storedAccount = safeStoreRead("QH");

  return {
    accounts: accountArgument || storedAccount || "",
...
function parseScriptArguments(value) {
  ...
  if (value && typeof value === "object") {
    if (Object.prototype.hasOwnProperty.call(value, "QH")) {
      return [value.QH, value.notify, value.debug].map((item) =>
        String(item ?? "")
      );
    }
...
function parseUsers(rawValue) {
  ...
  raw.split(/\r?\n/).forEach((rawLine, rawIndex) => {
    ...
    if (parts.length < 2 || !parts[0] || !parts[parts.length - 1]) {
      errors.push(`第${rawIndex + 1}条配置格式错误，应为 wid#openId`);
      return;
    }

    const wid = parts[0];
    const openId = parts[parts.length - 1];
    if (!openId.startsWith("o")) {
      errors.push(
        `第${rawIndex + 1}条配置的第二项不像 openId，请使用 wid#openId`
      );
      return;
    }

    users.push({ wid, openId });
  });
...
async function processUser(wid, openId, index) {
  const logs = [`账号${index}（wid=${wid}，openId=${shortOpenId(openId)}）`];
...
async function main() {
  const { users, errors: configErrors } = parseUsers(CONFIG.accounts);

  if (users.length === 0) {
    const detail = configErrors.length
      ? configErrors.join("；")
      : "未读取到账号配置";
    const message = `没有可用账号：${detail}。正确格式为 wid#openId`;
    console.log(message);
    postNotification(SCRIPT_NAME, "配置错误", message);
    return;
  }
  ...
  for (let index = 0; index < users.length; index += 1) {
    const { wid, openId } = users[index];
    console.log(`\n===== 开始处理账号 ${index + 1}（wid=${wid}）=====`);

    let logs;
    try {
      logs = await processUser(wid, openId, index + 1);
    } catch (error) {
      logs = [
        `账号${index + 1}（wid=${wid}，openId=${shortOpenId(openId)}）`,
    ...
    if (index < users.length - 1) await randomSleep(3, 5);
  }

  if (configErrors.length > 0) {
    allLogs.push(["配置提醒", ...configErrors]);
  }

  const report = renderReport(allLogs);
  console.log(`\n${report}`);
  postNotification(SCRIPT_NAME, `共处理 ${users.length} 个账号`, report);
}
...
(async () => {
  try {
    ...
    const message = `脚本运行失败：${error.message || error}`;
    console.log(`[${SCRIPT_NAME}] ${message}`);
    postNotification(SCRIPT_NAME, "运行失败", message);
  } finally {
    $done();
  }
})();
